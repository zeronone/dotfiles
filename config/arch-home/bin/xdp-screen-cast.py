...
def screen_cast_call(method, callback, *args, options={}):
    ...
    return method(*(args + (options, )), dbus_interface=screen_cast_iface)
...
def on_create_session_response(response, results):
    ...
    print("session %s created"%session, results)

    res = screen_cast_call(portal.SelectSources, on_select_sources_response,
                     session,
                     options={ 'multiple': True,
                               'types': dbus.UInt32(1|2) })
    print("SelectSources result: ", res)
